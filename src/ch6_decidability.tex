...
\begin{theorem}\label{computable-sum}
  ...
  Let $\qqq$ be a \emph{finite} set of computable properties of linear orders
  over some finite signature which is disjoint from the signature of $\pp$.

  Let $F : 2^k \to \qqq$ be any function.

  Then $\bigcup_{I \in \pp} \sum_{i \in I} F(C_1(i), \ldots, C_k(i))$ is a computable property of linear orders.
\end{theorem}
...
\begin{proof}
  ...
  Since $\qqq$ has only computable properties, We can pre-compute $\type{n}{F(\vec{c})}$
...
\begin{lemma}\label{many-ordinals-decomposition}
  ...
  such that $\alpha_{i} \ge \alpha_{i - 1} + \omega$ for $i > 0$.
  ...
  $\sigma(i) \in \set{1, \omega, \agemo, \oo}$ and
  $L(i) = 1$ iff $\sigma(i) \in \set{1, \omega}$ and
  $R(i) = 1$ iff $\sigma(i) \in \set{1, \agemo}$.
...
\begin{theorem}\label{many-ordinals-satisfiability}
  ...
  Satisfiability of $\mso[\mathbf{Int}_{\qqo^{< \alpha_1}}, \ldots, \mathbf{Int}_{\qqo^{< \alpha_k}}]$
  over all countable linear orders is decidable.
\end{theorem}
...
\begin{proof}
  ...
  any formula in
  \[
    \mso[\mathbf{Int}_{\qqo^{< \alpha_1}}, \ldots, \mathbf{Int}_{\qqo^{< \alpha_k}}]
  \]
