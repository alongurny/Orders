...
\begin{corollary}
  ...
  $\mso[\sim_{\alpha}, \sigma_{\alpha}]$.
\end{corollary}

\begin{proof}
  By definition, we can express $\bs{x}_{\alpha} \in \lb{\qqo^{< \alpha}}$ via
  $\sigma_{\alpha} \ps{x} \in \set{1, \omega}$
  and $\bs{x}_{\alpha} \in \rb{\qqo^{< \alpha}}$ via
  $\sigma_{\alpha} \ps{x} \in \set{1, \agemo}$.

  Now it follows from~\cref{alpha-expressible}.
\end{proof}
...
\begin{theorem}
  ...
  Then, $C = \sum_{\mathbf{G}} \bs{i \mapsto \mathcal{S}^{\sigma(i)}_{\alpha}}$.
\end{theorem}
...
\begin{proof}
  ...
  Suppose that there exist distinct $j ,k \in I$ (WLOG, $j < k$) such that
  $M_j$ and $M_k$ are in the same $\sim_{\alpha}$-equivalence class.
